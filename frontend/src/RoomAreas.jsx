...
export const BookShelfRef = forwardRef(function BookShelfRef({ highlighted }, ref) {
  const books = useMemo(() => {
    return Array.from({ length: 5 }, (_, i) => ({
      id: i,
      x: -0.5 + i * 0.18,
      height: 0.25 + Math.random() * 0.18,
      color: new THREE.Color().setHSL(0.06 + Math.random() * 0.12, 0.3, 0.22 + Math.random() * 0.08),
...
// ========================================
export const AnalysisDisplay = forwardRef(function AnalysisDisplay({ analysis, highlighted }, ref) {
  const wpm = Number(analysis?.latest_session?.wpm || 0);
    ...
    <group ref={ref} position={[3.95, 1.5, 0]}>
      ...
      {[0, 1, 2, 3, 4].map((i) => (
        <mesh key={i} position={[-0.04, -0.35 + i * 0.15, -0.6 + i * 0.12]} rotation={[0, -Math.PI / 2, 0]}>
          <boxGeometry args={[0.01, 0.06, 0.3 + Math.random() * 0.5]} />
          <meshStandardMaterial
...
// FRONT WALL (z = +3.5)
// ========================================
export const SuggestionCard = forwardRef(function SuggestionCard({ suggestion, highlighted }, ref) {
  return (
