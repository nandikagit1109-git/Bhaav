...
// ========================================
// SHARED: Glow ring for interactable objects
// ========================================
function GlowRing({ visible, color = '#4a6fa5', y = 0 }) {
  const ref = useRef();

  useFrame(() => {
    if (!ref.current) return;
    const target = visible ? 0.6 : 0;
    ref.current.material.opacity += (target - ref.current.material.opacity) * 0.08;
  });
...
export const Journal = forwardRef(function Journal({ highlighted }, ref) {
  ...
  useFrame((state) => {
    if (glowRef.current) {
      const mat = glowRef.current.material || glowRef.current;
  ...
  return (
    <group ref={ref} position={[0.15, 0.65, 0]}>
      <group ref={bodyRef}>
        ...
        <GlowRing visible={highlighted} y={-0.01} />
      </group>
    </group>
  );
});
...
export const DeskLamp = forwardRef(function DeskLamp({ highlighted }, ref) {
  ...
  useFrame((state) => {
    if (lightRef.current) {
      const target = highlighted ? 1.2 : 0.3;
      lightRef.current.intensity += (target - lightRef.current.intensity) * 0.04;
    }
    if (shadeRef.current) {
      const mat = shadeRef.current.material || shadeRef.current;
      if (mat && mat.emissiveIntensity !== undefined) {
        const target = highlighted ? 0.8 : 0.2;
        mat.emissiveIntensity += (target - mat.emissiveIntensity) * 0.04;
      }
    }
  });
  ...
  return (
    <group ref={ref} position={[-0.5, 0.62, -0.2]}>
      ...
      <GlowRing visible={highlighted} color="#e8c080" y={-0.005} />
    </group>
  );
});
...
export const Radio = forwardRef(function Radio({ highlighted }, ref) {
  ...
  useFrame(() => {
    ...
    const mat = bodyRef.current.material || bodyRef.current;
    if (mat && mat.emissiveIntensity !== undefined) {
  ...
  return (
    <group ref={ref} position={[1.2, 1.25, -1.75]}>
      ...
      <GlowRing visible={highlighted} color="#4a6fa5" y={-0.075} />
    </group>
  );
});
...
export const WallClock = forwardRef(function WallClock({ highlighted }, ref) {
  ...
  useFrame((state) => {
    if (handRef.current) {
      handRef.current.rotation.z = -state.clock.elapsedTime * 0.1;
    }
  });
  ...
  return (
    <group ref={ref} position={[-2.9, 1.8, -0.5]}>
      ...
      <GlowRing visible={highlighted} color="#4a6fa5" y={0} />
    </group>
  );
});

// ========================================
// FLOATING PAPERS (mood-reactive)
// ========================================
export function FloatingPapers({ clutterLevel = 0 }) {
  ...
  useFrame((state) => {
    ...
    groupRef.current.children.forEach((child, i) => {
      ...
      const amplitude = 0.1 + clutterLevel * 0.8;
      child.position.y = paper.pos[1] + Math.sin(t * paper.speed + paper.phase) * amplitude;
      child.rotation.x = paper.rot[0] + Math.sin(t * 0.3 + paper.phase) * 0.3 * clutterLevel;
      child.rotation.z = paper.rot[2] + Math.cos(t * 0.2 + paper.phase) * 0.2 * clutterLevel;
    });
  });
  ...
  return (
    <group ref={groupRef}>
      {papers.map((paper) => (
        <mesh key={paper.id} position={paper.pos} rotation={paper.rot}>
          <planeGeometry args={[0.18, 0.24]} />
          <meshStandardMaterial
            color="#1a1816"
            side={THREE.DoubleSide}
            transparent
            opacity={0.25 + clutterLevel * 0.4}
          />
        </mesh>
      ))}
    </group>
  ...
  return (
    <group ref={ref} position={[0, 1.15, -1.85]}>
      ...
      {books.map((book) => (
        <mesh
          key={book.id}
          position={[book.x, book.height / 2 + 0.02, 0]}
          rotation={[0, 0, book.lean + clutterLevel * (Math.random() - 0.5) * 0.2]}
          castShadow
        >
      ...
      <GlowRing visible={highlighted} color="#4a6fa5" y={-0.02} />
    </group>
  );
});
...
export const Window = forwardRef(function Window({ moodLevel = 0, highlighted }, ref) {
  ...
  useFrame(() => {
    if (lightRef.current) {
      const target = moodLevel * 0.6;
      lightRef.current.intensity += (target - lightRef.current.intensity) * 0.02;
    }
    if (paneRef.current) {
      const mat = paneRef.current.material || paneRef.current;
      if (mat && mat.emissiveIntensity !== undefined) {
...
export function WallArt() {
  ...
  useFrame((state) => {
    if (frameRef.current) {
      const breathe = Math.sin(state.clock.elapsedTime * 0.3) * 0.05 + 0.3;
      frameRef.current.material.emissiveIntensity = breathe;
    }
  });
  ...
  return (
    <group position={[-2.95, 1.8, 0.5]}>
      ...
      {/* Canvas — warm gradient feel */}
      <mesh ref={frameRef} position={[0.015, 0, 0]}>
...
export function PottedPlant() {
  ...
  useFrame((state) => {
    if (leavesRef.current) {
      ...
      leavesRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 0.3) * 0.02;
    }
  });
  ...
  return (
    <group position={[1.8, 0.62, 0.2]}>
      ...
      {/* Leaves — green color pop */}
      <group ref={leavesRef} position={[0, 0.12, 0]}>
        {[0, 1.2, 2.4, 3.6, 4.8].map((angle, i) => (
          <mesh
            key={i}
            position={[Math.cos(angle) * 0.04, 0.05 + i * 0.03, Math.sin(angle) * 0.04]}
            rotation={[0.3 + i * 0.1, angle, 0.1]}
          >
            <planeGeometry args={[0.06, 0.08]} />
            <meshStandardMaterial
              color={i % 2 === 0 ? '#2d5a3a' : '#3a6b4a'}
              emissive={i % 2 === 0 ? '#1a3a25' : '#2a4a35'}
              emissiveIntensity={0.3}
              roughness={0.7}
              side={THREE.DoubleSide}
            />
...
// ========================================
// WALL LIGHT STRIP — warm amber accent
// ========================================
